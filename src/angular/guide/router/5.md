...
affects the router's navigation behavior:
{% comment %} FIXME: the following doesn't hold for all hooks -- e.g., canDeactivate. {% endcomment %}
...
<?code-excerpt "lib/src/crisis/crisis_component.dart (onActivate)" remove="·" replace="/name = .*/[!$&!]/g" title?>
```
...
<?code-excerpt "lib/src/crisis/crisis_component.dart (save)" title?>
```
...
Next, implement a router [CanDeactivate][] lifecycle hook in `CrisisComponent`:
...
- Add the following `canDeactivate()` lifecycle hook:

<?code-excerpt "lib/src/crisis/crisis_component.dart (canDeactivate)" title?>
```
...
Since the router calls the `canDeactivate()` method when necessary, so you don't
...
{%- comment %}
TODO: revert print statements to their simpler form or update the JS console output below.
{% endcomment %}

<?code-excerpt "lib/src/crisis/crisis_component.dart (excerpt)" region="OnActivate-and-OnDeactivate" replace="/log/print/g" title?>
```
  CrisisComponent(this._crisisService, this._router, this._diaprintService) {
    print('created');
  }
  // ···
  Future<void> onActivate(_, RouterState current) async {
    print('onActivate: ${_?.toUrl()} -> ${current?.toUrl()}');
    // ···
  }
  // ···
  void onDeactivate(RouterState current, _) {
    print('onDeactivate: ${current?.toUrl()} -> ${_?.toUrl()}');
  }
```
...
CrisisComponent: canDeactivate: crises/1 -> crises/2
CrisisComponent: onDeactivate: crises/1 -> crises/2
CrisisComponent: created
CrisisComponent: onActivate: crises/1 -> crises/2
CrisisComponent: canDeactivate: crises/2 -> crises/3
...
<?code-excerpt "lib/src/crisis/crisis_component.dart (CanReuse)" replace="/extends \w+|with \w+/[!$&!]/g; /, InstanceLogger//g" title?>
```
